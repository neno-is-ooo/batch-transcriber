...
from faster_whisper_batch.events import EventEmitter
...
class EventEmitterTests(unittest.TestCase):
    def test_emits_protocol_events_as_ndjson_lines(self) -> None:
        ...
        with redirect_stdout(stream):
            emitter.emit_start("session-1", "faster-whisper", "large-v3", "cuda", "float16")
            emitter.emit_models_loaded()
            emitter.emit_file_started(0, 2, "/audio/a.wav", "a.wav")
            emitter.emit_file_progress(0, "/audio/a.wav", 50.0, 2.8)
            emitter.emit_file_retry(
                index=0,
                file="/audio/a.wav",
                reason="GPU OOM, retrying with int8_float16",
                device="cuda",
            ...
            emitter.emit_file_failed(1, "/audio/b.wav", "decode failed", 2)
        ...
        self.assertEqual(events[4]["event"], "file_retry")
        self.assertEqual(events[5]["event"], "file_done")
        self.assertEqual(events[6]["event"], "file_failed")
        self.assertEqual(events[7]["event"], "file_skipped")
        self.assertEqual(events[8]["event"], "fatal_error")
        self.assertEqual(events[9]["event"], "summary")
        ...
        start = events[0]
        self.assertEqual(start["device"], "cuda")
        self.assertEqual(start["compute_type"], "float16")

        retry = events[4]
        self.assertEqual(retry["device"], "cuda")
        self.assertEqual(retry["compute_type"], "int8_float16")
